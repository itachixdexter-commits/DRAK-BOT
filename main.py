...
import random
import os
import openai # Added for AI chat
...
VIRUSTOTAL_API_KEY = "b8163d15425405d2ee349307c044811bc0955078fb7f1057bc99d3dd216bb1bb"
OPENAI_API_KEY = "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" # REPLACE WITH YOUR ACTUAL OPENAI API KEY
...
async def start(update: Update, context) -> None:
    keyboard = [
        ...
        [InlineKeyboardButton("تكلم مع AI", callback_data=\'toggle_ai_chat\')], # Added AI chat button
    ]
...
async def button_callback(update: Update, context) -> None:
    ...
    elif query.data == \'toggle_ai_chat\': # Handle AI chat toggle
...
async def get_ip_information(update: Update, context) -> None:
    ...
    try:
        async with httpx.AsyncClient() as client:
            response = await client.get(f"http://ip-api.com/json/{ip_address}?lang=ar")
        response.raise_for_status()
        data = response.json()
...
async def deobfuscate_roblox_script(update: Update, context) -> None:
    ...
    max_iterations = 10 # Increased iterations for more aggressive decoding

    await update.message.reply_text("جاري محاولة فك تشفير السكربت بأساليب متقدمة...")
    ...
    for i in range(max_iterations):
        ...
        try:
            # Check for common Base64 patterns, often padded with =
            if re.match(r"^[A-Za-z0-9+/=]+\s*$", decoded_content.strip()) and len(decoded_content.strip()) % 4 == 0:
                decoded_bytes = base64.b64decode(decoded_content, validate=True)
                decoded_content = decoded_bytes.decode(\'utf-8\', errors=\'ignore\')
                await update.message.reply_text(f"تم فك تشفير Base64 (الطبقة {i+1}).")
        except Exception:
            pass
        ...
        try:
            if re.match(r"^[0-9a-fA-F]+\s*$", decoded_content.strip()) and len(decoded_content.strip()) % 2 == 0:
                ...
                await update.message.reply_text(f"تم فك تشفير Hex (الطبقة {i+1}).")
        except Exception:
            pass
        ...
        try:
            if "%" in decoded_content:
                ...
                await update.message.reply_text(f"تم فك تشفير URL (الطبقة {i+1}).")
        except Exception:
            pass

        # Clean up common escape sequences (often used after decoding)
        decoded_content = decoded_content.replace("\\n", "\n").replace("\\t", "\t").replace("\\\"", "\"").replace("\\\\\'", "\'")

        # Attempt Zlib decompression (often follows Base64 or Hex)
        try:
            # Zlib magic numbers: 0x78 0x9C (DEFLATE), 0x78 0xDA (DEFLATE, best compression)
            # Need to encode to bytes for zlib
            if decoded_content.startswith("\x78\x9c") or decoded_content.startswith("\x78\xda"):
                ...
                await update.message.reply_text(f"تم فك ضغط Zlib (الطبقة {i+1}).")
            # Also try if it\'s a raw deflate stream (without zlib header)
            elif len(decoded_content) > 0:
                try:
                    decompressed_data = zlib.decompress(decoded_content.encode(\'latin-1\'), -zlib.MAX_WBITS)
                    decoded_content = decompressed_data.decode(\'utf-8\', errors=\'ignore\')
                    await update.message.reply_text(f"تم فك ضغط Deflate خام (الطبقة {i+1}).")
                except zlib.error:
                    pass
        except Exception:
            pass

        # If no change occurred in this iteration, break the loop
...
async def scan_url_function(update: Update, context) -> None:
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            ...
            await update.message.reply_text("🔍 جاري تحليل الرابط، انتظر قليلاً...")
            ...
            while True:
                ...
                if report_data["data"]["attributes"]["status"] == "completed":
                    break
                elif report_data["data"]["attributes"]["status"] == "queued":
                    await update.message.reply_text("التحليل في قائمة الانتظار، جاري الانتظار...")
                else:
                    await update.message.reply_text(f"حالة التحليل غير متوقعة: {report_data[\"data\"][\"attributes\"][\"status\"]}")
                    context.user_data["state"] = None
                    return

            # Step 3: Get the actual URL report for detailed stats
            # VirusTotal\'s analysis endpoint only gives a summary, we need the files/urls endpoint for full details
            # First, get the URL ID from the analysis report
            url_id = base64.urlsafe_b64encode(url.encode()).decode().strip("=")

            final_report_response = await client.get(
            ...
            stats = final_report_data["data"]["attributes"]["last_analysis_stats"]
            ...
            msg = (
                ...
                f"☠️ خبيث: {malicious}\n"
                f"❓ غير مكتشف: {undetected}"
            )

            await update.message.reply_text(msg)

    except httpx.RequestError as e:
        await update.message.reply_text(f"❌ خطأ في الاتصال بـ VirusTotal: {e}")
    except json.JSONDecodeError:
        await update.message.reply_text("❌ خطأ في تحليل استجابة VirusTotal.")
    except KeyError as e:
        await update.message.reply_text(f"❌ خطأ في تحليل بيانات VirusTotal (مفتاح مفقود): {e}")
    except Exception as e:
        await update.message.reply_text(f"❌ خطأ غير متوقع أثناء الفحص: {e}")
...
async def toggle_ai_chat(update: Update, context) -> None:
    ...
    if context.user_data.get("ai_chat_enabled", False):
        context.user_data["ai_chat_enabled"] = False
        await update.callback_query.edit_message_text("🤖 وضع التحدث مع AI معطل الآن. يمكنك تفعيله مرة أخرى من قائمة /start.")
    else:
        context.user_data["ai_chat_enabled"] = True
        await update.callback_query.edit_message_text("🤖 وضع التحدث مع AI مفعل الآن. أرسل لي أي شيء وسأرد عليك. لإيقاف الوضع، أرسل كلمة \"off\".")

async def ai_chat_response(update: Update, context) -> None:
    user_message = update.message.text
    if user_message.lower() == "off":
        context.user_data["ai_chat_enabled"] = False
        await update.message.reply_text("🤖 تم تعطيل وضع التحدث مع AI.")
        return
    ...
    try:
        ...
        response = openai.ChatCompletion.create(
            ...
            messages=[
                {"role": "system", "content": "أنت مساعد ذكاء اصطناعي مفيد وودود."},
        ...
        await update.message.reply_text(f"❌ حدث خطأ أثناء التواصل مع AI: {e}")
